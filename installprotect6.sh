...
echo "🔄 Updating all protections with JEREXD watermark..."

# Update PROTECT 6 dengan versi yang sudah diperbaiki
cat > /tmp/installprotect6_fixed.sh << 'EOF'
#!/bin/bash
...
# Update PROTECT 7 dengan fungsi unarchive/archive
cat > /tmp/installprotect7_fixed.sh << 'EOF'
#!/bin/bash
...
bash /tmp/installprotect6_fixed.sh
bash /tmp/installprotect7_fixed.sh

echo "🎉 All protections updated successfully!"
echo "🔧 Restart panel: cd /var/www/pterodactyl && php artisan optimize:clear"
